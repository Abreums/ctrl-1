# Quantos Km foram planejados executados? ------ 
#
# pr3f_km.R
#
...
# 2. Vamos ler os dados do PowerBI (origem dos dados SAP-PS) ---------------
...
sumario_km <- function(pr3f) {
  ...
  pr3f <- pr3f %>%                     # Dados sumarizados 1 linha por obra
    filter(tipo_de_obra == "Investimento", # Exclui Topografias e Projetos
           estagio != "CANCELADO") %>%     # Exclui projetos CANCELADOs
    group_by(id_da_obra) %>% 
    summarize(dpo = first(dpo),
              vpo = first(vpo),
              po = first(po),
              km_planejado = first(km_planejado),
              km_projeto = sum(km_projeto),
              km_concluido = sum(km_concluido),
              n_prj = n()))
}
pr3f <- sumario_km(pr3f)
pr3f %>% count(po, km_planejado)
