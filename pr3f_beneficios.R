...
obras <- read_bi_obras(obras_file)

# Sumarizar 1 linha por Obra - PR3F ----
sumario_pr3f <- function(pr3f) {
  sum_pr3f <- pr3f %>%
    filter(estagio %in% c("CONC/ENCE", "CONC_BMD")) %>% 
    dplyr::select(id_da_obra, projeto, km_concluido, dt_fim_real) %>% 
    na.omit() %>% 
    arrange(dt_fim_real) %>% 
    mutate(dt_fim_real = as.Date(dt_fim_real)) %>% 
    #count(id_da_obra, wt = km_concluido, name = "km_entregues")
    group_by(id_da_obra) %>% 
    summarize(km_entregues = sum(km_concluido),
              dt_fim_real = max(as.Date(dt_fim_real)))
    
}
sum_pr3f <- sumario_pr3f(pr3f)


# Sumarizar 1 linha por Obra  ----
sumario_obras <- function(obras) {
  obras <- obras %>%
    group_by(id_da_obra) %>% 
...
sum_obras <- sumario_obras(obras)

km_entregues_por_obra_concluida <- function(sum_obras, sum_pr3f) {
  sum_obras <- sum_obras %>% 
    left_join(sum_pr3f,
              by = c("id_da_obra" = "id_da_obra")) %>% 
    filter(situacao %in% c("Concluído"))
}
concluidas <- km_entregues_por_obra_concluida(sum_obras, sum_pr3f)

# 2. Ler Alim x APD x Obras ----
read_al_apd_obra <- function() {
  # Busca lista de obras x alimentadores
  al_apd_obra <- read_xlsx(path = "./data/alim_apd_id_da_obra.xlsx",
                           sheet = 1)
...
# join alim x geo -----
join_alim_geo <- function(alim, geo) {
  al_obra <- alim %>% 
    left_join(geo, 
            by = c("num_al_eletrico" = "num_alim_gdmase"))
}
al_obra <- join_alim_geo(al_obra, alim_geo)


# 4 join concluídas x geo -----
concluidas <- concluidas %>% 
  left_join(al_obra,
            by = c("id_da_obra" = "id_da_obra"))

# Nem toda obra está vinculada a um alimentador -------
# Por isso há obras sem coordenadas
# Por exemplo, obras de linhas, ou estações, ou
# ...ou obras de novos alimentadores.
concluidas %>% visdat::vis_dat()
concluidas %>% visdat::vis_miss()
...
# Obras sem coordenadas, vamos utilizar as do centroide do município 
concluidas <- concluidas %>% 
...
separa_info_id_obra <- function(df) {
  ...
  df <- df %>% 
    mutate(regional = factor(regional))
}
concluidas <- separa_info_id_obra(concluidas)
concluidas %>% vis_dat()
...
(myplot <- ggplot() +
    ...
    labs(x = NULL,
         ...
         subtitle = "Obras Entregues - Status  2021-01-28",
